...
import { getMenuItems } from "./sidemenu/sidemenu";
...
const Sidebar = ({ local_varaiable, ThemeChanger }) => {
	...
	useEffect(() => {
		...
		const handleStorageChange = (e) => {
			if (e.key === 'usuario' || e.key === 'tempUser') {
				updateMenu();
			}
		};
	...
	const setMenuUsingUrl = (currentPath) => {
	  ...
	  setSubmenuRecursively(getMenuItems());
	};
	...
	const setMenuAncestorsActive = (targetObject) => {
	  const parent = findParent(getMenuItems(), targetObject);
	  const theme = store.getState();
	  if (parent) {
		if (hasParentLevel > 2) {
			hasParent = true;
